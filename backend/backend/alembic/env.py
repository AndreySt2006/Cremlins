...
# Make sure backend package (project backend folder) is importable
# env.py lives in backend/backend/alembic/; backend project root is two levels up
here = Path(__file__).resolve()
project_backend_dir = here.parents[2]
sys.path.insert(0, str(project_backend_dir))

# Import metadata from models for 'autogenerate'
try:
